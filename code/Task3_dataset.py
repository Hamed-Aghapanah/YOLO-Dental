...
for _, row in df.iterrows():
    ...
    if code:
        code_to_class[code] = 'wisdom_tooth'
...
# ==============================
def split_and_save_image(img_path, filename, output_dir, set_name):
    ...
    # ذخیره نیم تصاویر
    left_filename = f"{base_name}_L.png"
    right_filename = f"{base_name}_R.png"

    left_label_file = f"{base_name}_L.txt"
    right_label_file = f"{base_name}_R.txt"
...
for set_name, images in [('train', train), ('val', val), ('test', test)]:
    for img_name in tqdm(images, desc=f"Processing {set_name}"):
        ...
        split_and_save_image(img_path, img_name, OUTPUT_DIR, set_name)
