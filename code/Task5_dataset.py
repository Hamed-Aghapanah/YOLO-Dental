...
LOCALIZATION_LABELS_DIR = "path/to/localization_labels"  # فایل‌های خروجی Task2/3
OUTPUT_DIR = "Task5_WITH_LOCAL_CLASSIFICATION"
...
# ==============================
# تابع برای خواندن لیبل و برش تصویر
# ==============================
def crop_and_save_image(img_path, filename, output_dir, label, set_name):
    ...
    # خواندن لیبل (YOLO format)
    label_file = os.path.join(LOCALIZATION_LABELS_DIR, f"{base_name}.txt")
    ...
    with open(label_file, 'r') as f:
        lines = f.readlines()
        if len(lines) == 0:
            print(f"No bounding box in label file: {label_file}")
            return
    ...
    if cropped_img.size == 0:
        print(f"Cropped image is empty: {filename}")
        return
...
for set_name, images in [('train', train), ('val', val), ('test', test)]:
    for img_name in tqdm(images, desc=f"Processing {set_name}"):
        ...
        crop_and_save_image(img_path, img_name, OUTPUT_DIR, label, set_name)
